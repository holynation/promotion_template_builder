...
$tableData='';
$roleFaculty=$currentRole->getFaculty(true);
$roleDepartment=$currentRole->getDepartment(true);
if ($model=='student_biodata') {
  if ($roleFaculty) {
   $qTemp= combineForInQuery(fetchField($roleFaculty,'faculty_id'));
   $where.=$where?" and faculty.id in $qTemp ":" where faculty.id in $qTemp ";
  }
  if ($roleDepartment) {
   $qTemp= combineForInQuery(fetchField($roleDepartment,'department_id'));
   $where.=$where?" and department.id in $qTemp ":" where department.id in $qTemp ";
  }
...
if ($model=='request') {
  if ($roleDepartment) {
   $qTemp= combineForInQuery(fetchField($roleDepartment,'department_id'));
   $inQuery=" select distinct student.ID from student_biodata student join program on program.id= student.program_id join department on department.id=program.department_id where department.id in $qTemp ";
   $where.=$where?" and user.user_type='student_biodata' and user.user_table_id in ($inQuery) ":" where user.user_type='student_biodata' and user.user_table_id in ($inQuery) ";
  }
}
 ...
 <!-- Main content -->
  ...
  <section class="content">
      ...
      <div class="top-action pull-right">
        ...
        <?php if ($configData==false || array_key_exists('has_upload', $configData)==false || $configData['has_upload']): ?>
           | <span class="main-menu-item" data-toggle='modal' data-target='#modal-upload'>Batch Upload</span>
        <?php endif ?>
      ...
      <div class="filter-section">
        ...
        <?php if ($search): ?>
          <?php 
            ...
            $val = $this->db->conn_id->escape_string($val);
           ?>
          <input class="form-control1" type="text" name="q" placeholder="<?php echo $placeholder ?>" value="<?php echo $val ?>">
